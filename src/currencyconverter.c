...
static void updateCurrentCurrency() {
  ...
  strncpy(currency_current, &currencies[currency_index * CURRENCY_STR_LEN],
          CURRENCY_STR_LEN);

  persist_read_string(currency_index + CURRENCY_OFFSET + 1, rate, sizeof(rate));
  APP_LOG(APP_LOG_LEVEL_DEBUG, "persist-key: %d : %s (%s)",
          (int)currency_index + CURRENCY_OFFSET + 1, currency_current, rate);
...
static void in_received_handler(DictionaryIterator *received, void *context) {
  ...
  while (tuple) {
    // APP_LOG(APP_LOG_LEVEL_DEBUG, "Tuple: %d : %s", (int)tuple->key,
    // tuple->value->cstring);
    switch (tuple->key) {
      case CURRENCY_INDEX:
...
static void click_config_provider(void *context) {
  ...
  window_multi_click_subscribe(BUTTON_ID_SELECT, 2, 10, 0, true,
...
static void window_load(Window *window) {
  ...
  // Equals sign
  from_layer = text_layer_create(
      (GRect) {.origin = {0, 20}, .size = {bounds.size.w, 35}});
  equals_layer = text_layer_create(
      (GRect) {.origin = {0, center.y - 20}, .size = {bounds.size.w, 35}});
  to_layer = text_layer_create(
      (GRect) {.origin = {0, bounds.size.h - 60}, .size = {bounds.size.w, 35}});
  no_currency_layer = text_layer_create((GRect) {
      .origin = {center.x, bounds.size.h - 20}, .size = {center.x, 20}});

  text_layer_set_text(from_layer, "No currencies");
  text_layer_set_font(from_layer,
  ...
  text_layer_set_font(equals_layer,
                      fonts_get_system_font(FONT_KEY_GOTHIC_28_BOLD));
